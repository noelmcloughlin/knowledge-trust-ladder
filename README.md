...
**What each skill needs.** Every skill runs from a POSIX shell and starts with a preflight that prints what this machine can do and which steps that disables. Someone who cannot act on a line it prints - a curator who knows the subject, not the repository - gets a request note for whoever set the repository up, from the sidecar's [prerequisites page](skills/lokf-sidecar/references/prerequisites.md); what the curator needs to record a confirmation in your name is in [docs/signing-commits.md](docs/signing-commits.md).
