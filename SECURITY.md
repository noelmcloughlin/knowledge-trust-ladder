...
| --- | --- |
| `skills/lokf-sidecar/templates/` - the wrapper script and two workflows the sidecar **copies into other repositories**, which run there | The template's own design: two jobs so the agent never meets a write token, and a `publish` job that confines the patch to the bundle and refuses a `human:` claim. [Prompt-injection guards](docs/threat-model.md#prompt-injection-guards). |
