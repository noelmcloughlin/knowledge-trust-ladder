...
Where no gate runs, or as a second opinion where one does, lokf-sidecar can lay down `knowledge-provenance.sh` beside the conventions script: it verifies the signature on every commit that adds a `human:<id>` line against a public key the repository carries for that id under `.lokf/curators/`, with plain git and gpg, on any CI or by hand. Its rules, and the guard that stops a change adding a key and a confirmation together, are in lokf-sidecar's [portability.md](../../lokf-sidecar/references/portability.md).
