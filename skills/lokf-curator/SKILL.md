...
8. **Ready to record** - from the preflight: the id a confirmation would carry and where it came from, signing on or off, attended or not. When any is missing, say here which verbs that removes ("no authenticated login: I can send back, retire or mark for later; to confirm in your name, `gh auth login` first, or one of the routes in [references/portability.md](references/portability.md)"), so nothing below offers what Step 2 cannot do.
