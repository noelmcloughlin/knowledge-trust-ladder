...
1. **File → Open folder as vault**, and pick `knowledge_bundle` at the host root - the link `ktl-sidecar` laid beside `.lokf/` so the bundle has a
...
- **Making the link yourself.** This skill is read-only outside `.lokf/feedback.md`, and a missing link is not a knowledge gap - never record it
  there. Give the command; if they want an agent to do it, that is `ktl-sidecar`'s repair path (`just lokf-link`).
