...
| `templates/scripts/knowledge-provenance.sh` | `.lokf/scripts/knowledge-provenance.sh` (`chmod +x`) - the signature half of the gate on any host with git and gpg; verifies only once `.lokf/curators/<id>.asc` keys exist ([references/portability.md](references/portability.md)) |
