...
The `provenance` job above needs GitHub. Where it cannot run, and as a second opinion where it does, this script verifies the signature on every commit in a range that adds a `by: human:<id>` line under the bundle against a public key the repository carries for that id - `.lokf/curators/<id>.asc`, the `gpg --armor --export` of the curator's key - with plain git and gpg. Unsigned, signed by another key, or an id with no key on file each fail it, and so does a range that adds or changes a key file and a confirmation together, so a key lands in its own reviewed change before anyone vouches with it. With no `.lokf/curators/` it says so and passes: a host that has not opted in loses nothing. The GitHub job runs it as an extra step when the directory exists; on another forge or a plain CI, run it with `<base> <head>` from the pull or merge request. SSH signatures are reported as unsupported for now; GPG ones are verified. It proves who held the key, never that anyone read the source.
