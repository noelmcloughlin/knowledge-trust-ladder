...
**Skip the workflows and the wrapper if `.lokf/` is gitignored.** The workflows need the bundle on a remote branch, and the librarian loop's `git status --porcelain` check silently reports "no changes" for an ignored path forever. Two scripts and the `m365/` folder land regardless, and their rows say why: the preflight, `knowledge-feedback.sh`, and the Copilot skills' builder with its instructions. The workflows are GitHub-only; on other hosts, copy just the wrapper and schedule it with cron or CI (see [references/portability.md](references/portability.md)). None of these files has placeholders.
...
| `templates/github/knowledge-release.yaml` | `.github/workflows/knowledge-release.yaml`: attaches the bundle to a GitHub release as a zip file, with one Copilot skill zip per file under `.lokf/m365/` beside it, and skips a release whose bundle matches the last one released. It runs when dispatched by hand, and on each published release once the `KNOWLEDGE_RELEASE_ENABLED` variable is `true`; it needs no agent |
...
| `templates/m365/knowledge-m365.sh` | `.lokf/m365/knowledge-m365.sh` (`chmod +x`): builds one Microsoft 365 Copilot custom skill per instructions file beside it, each with a snapshot of the bundle inside. The release workflow runs it and attaches each zip beside the bundle zip; what Copilot allows and what each role can be there: [references/m365.md](references/m365.md). Lay it down even when the rest of this step is skipped: a host with no git builds the skill by hand and uploads it, and it needs neither git nor GitHub |
