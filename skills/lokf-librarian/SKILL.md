...
bash scripts/knowledge-conventions.sh   # log headings, quoted timestamps, verified lists, open-question shape, local resources exist, revisions hold their resource, one file per id, lowercase paths, readable frontmatter (the YAML rules run through uv; without uv it says which it skipped)
