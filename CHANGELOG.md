...
- **A forge-free provenance gate.** `knowledge-provenance.sh` verifies each confirmation's commit signature against a public key the repository carries per curator id under `.lokf/curators/`, with plain git and gpg on any host, and refuses a key and a confirmation landing in one change. The GitHub job runs it as a second opinion when the directory exists. Check 13 proves the pass, the three failures and the skip with throwaway keys.
