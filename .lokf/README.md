...
This is a small **sidecar** that captures Knowledge Trust Ladder's own knowledge (its services, metrics, policies, playbooks, and glossary) as plain Markdown files that are **also a queryable knowledge graph**. It does not touch the app build; it is independent tooling you can run on its own.
