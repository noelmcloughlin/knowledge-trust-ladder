...
layout tests, the paths three sibling repositories deep-link into by URL
still existing - and, when those siblings are cloned beside this repo, that
the list of them is complete - and `CONTRIBUTING.md` staying under its
1000-word budget (added 2026-09-14). Then `gh skill publish --dry-run` runs.

`lint-scripts` runs ShellCheck on every script; `lint-workflows` runs
`actionlint`, which also reaches the two workflow templates that get copied
into other repositories, and (added 2026-09-14) a check that every `uses:`
