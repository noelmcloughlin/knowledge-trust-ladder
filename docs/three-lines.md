...
| **Second**: ensures compliance, without owning the content | risk and compliance functions | The **registrar** keeps every record well-formed: `lokf validate` on every change and again as the CI gate, and the KTL Registrar plugin in Obsidian. The gate's `provenance` job ties each new `human:` verdict to that person's approval of the pull request or their signature on the commit. It never judges truth: a verdict is only ever what that person said. |
...
- A confirmation records who and when. It records which state of the source only when the event carries `revision`, and even then the state the skill fetched, not what the person read.
