...
| Which state of the source was the check made against? | `verified[].revision` and `generated.revision` (lokf 0.9.0+): a commit id for a file in the repository, an ETag or content digest for a URL. Absent means unrecorded, never unchanged |
